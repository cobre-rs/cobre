...
| F3-006 | **SKIPPED** | month0() hardcoding in window.rs |
...
### F3-006: month0() hardcoding in window.rs
**Location:** `cobre-stochastic/src/sampling/window.rs:140-148`
**Why skipped:** The `month0()` call maps dates to season IDs using calendar
months (0 = January). This works for monthly study cycles (the only cycle
currently used in production). For weekly or custom-defined cycles, the
season derivation should use `SeasonMap::season_for_date()` instead. However:
1. `SeasonMap` is not currently passed to `discover_historical_windows`.
2. No test case or user has encountered a non-monthly cycle yet.
3. The fix requires threading `SeasonMap` through the function signature
   and updating all call sites.
**Recommended approach:** When adding weekly/custom cycle support, pass
`SeasonMap` to `discover_historical_windows` and replace `month0()` with
`season_map.season_for_date(&r.date)`. Add a test with a custom 4-season
(quarterly) cycle to verify.
