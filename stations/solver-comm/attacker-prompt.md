...
Baseline: `{{BASELINE_SHA}}`   Station: `solver-comm`   Lens: `{{LENS}}`
...
for this run is `{{LENS}}`; the other three lenses are covered by sibling workers, so stay inside
  ...
  "candidates": [
    {
      ...
      "evidence": {
        "command": "git show {{BASELINE_SHA}}:crates/cobre-solver/src/types.rs | grep -n 'pub n_hydro'",
