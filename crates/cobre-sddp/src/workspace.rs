...
pub(crate) struct BackwardAccumulators {
    ...
    /// `BackwardBasisStore::write_slot_mut(s)`.  Mirrors the
...
/// Per-stage double-buffer basis cache for the backward pass ω=0 warm-start.
///
/// Implements the double-buffer design from master plan AD-1 and AD-4:
/// one buffer (`read_buf`) is read by the current iteration's backward ω=0
/// solve, and the other (`write_buf`) is populated by rank-0's m=0 capture
/// during the same iteration. After the iteration's MPI broadcast, the
/// buffers are swapped and the new write buffer is cleared, making the
/// just-captured bases available for the next iteration's reads.
///
/// # Lifecycle
///
/// 1. Allocate once per `train` call: `BackwardBasisStore::new(num_stages)`.
/// 2. Iteration 1: both buffers start as `None`; the ω=0 solve falls through
///    to the forward [`BasisStore`] fallback (ticket-003).
/// 3. After each backward pass, rank-0 writes via [`write_slot_mut`].
/// 4. After broadcast (ticket-004), call [`swap_buffers`] once to promote
///    the captured bases to the read side.
///
/// [`write_slot_mut`]: BackwardBasisStore::write_slot_mut
/// [`swap_buffers`]: BackwardBasisStore::swap_buffers
#[derive(Debug)]
pub struct BackwardBasisStore {
    /// Read side: bases available for the current iteration's ω=0 warm-start.
    read_buf: Vec<Option<CapturedBasis>>,
    /// Write side: bases being captured during the current iteration's m=0 pass.
    write_buf: Vec<Option<CapturedBasis>>,
    /// Number of stages this store was sized for.
    num_stages: usize,
}

impl BackwardBasisStore {
    /// Allocate a new store for `num_stages` stages with every slot in both
    /// buffers initialized to `None`.
    ///
    /// Iteration 1 will always read `None` from the read buffer; the ω=0 solve
    /// must fall through to the forward [`BasisStore`] fallback (ticket-003).
    #[must_use]
    pub fn new(num_stages: usize) -> Self {
        Self {
            read_buf: vec![None; num_stages],
            write_buf: vec![None; num_stages],
            num_stages,
        }
    }

    /// Return the basis for `stage` from the read buffer (current iteration).
    ///
    /// Returns `None` if the slot has not yet been populated (e.g. iteration 1,
    /// or after a `swap_buffers` that cleared the write side before its first
    /// capture).
    #[must_use]
    pub fn get(&self, stage: usize) -> Option<&CapturedBasis> {
        debug_assert!(
            stage < self.num_stages,
            "stage {stage} out of bounds (num_stages={})",
            self.num_stages
        );
        self.read_buf[stage].as_ref()
    }

    /// Return a mutable reference to the write-buffer slot for `stage`.
    ///
    /// Used by rank-0's m=0 capture site (ticket-002) to install a freshly
    /// captured [`CapturedBasis`] into the pending-write side.
    pub fn write_slot_mut(&mut self, stage: usize) -> &mut Option<CapturedBasis> {
        debug_assert!(
            stage < self.num_stages,
            "stage {stage} out of bounds (num_stages={})",
            self.num_stages
        );
        &mut self.write_buf[stage]
    }

    /// Borrow the entire write buffer immutably.
    ///
    /// Used by the broadcast helper (ticket-004) on rank-0 to enumerate the
    /// captured bases for transmission to non-root ranks.
    #[must_use]
    pub fn write_buf(&self) -> &[Option<CapturedBasis>] {
        &self.write_buf
    }

    /// Replace the entire write buffer with `buf`.
    ///
    /// Used by the broadcast helper (ticket-004) on non-root ranks to install
    /// the canonical cache received from rank-0.  The incoming vector must have
    /// exactly `num_stages` elements.
    pub fn replace_write_buf(&mut self, buf: Vec<Option<CapturedBasis>>) {
        debug_assert!(
            buf.len() == self.num_stages,
            "replace_write_buf: buf.len()={} != num_stages={}",
            buf.len(),
            self.num_stages,
        );
        self.write_buf = buf;
    ...
    /// Swap the read and write buffers, then clear every slot in the new
    /// write buffer.
    ///
    /// Call this once per iteration after the broadcast completes.  On return:
    /// - `read_buf` holds the bases just captured/received (ready for the next
    ///   iteration's ω=0 warm-start).
    /// - `write_buf` is all-`None` (ready for the next iteration's captures).
    ///
    /// Clearing the new write buffer drops any previously-held [`CapturedBasis`]
    /// values, releasing their memory if the slot is not overwritten before the
    /// next swap.
    pub fn swap_buffers(&mut self) {
        std::mem::swap(&mut self.read_buf, &mut self.write_buf);
        for slot in &mut self.write_buf {
            *slot = None;
        }
    }

    /// Borrow the read buffer immutably.
    ///
    /// Used to build a `SuccessorSpec.backward_store: &[Option<CapturedBasis>]`
    /// slice for ω=0 warm-starts from the previous iteration's captures.
    #[must_use]
    pub fn read_buf(&self) -> &[Option<CapturedBasis>] {
        &self.read_buf
    }

    /// Return the number of stages this store was sized for.
    #[must_use]
    pub fn num_stages(&self) -> usize {
        self.num_stages
    }
...
mod tests {
    use super::{
        BackwardBasisStore, BasisStore, CapturedBasis, ScratchBuffers, SolverWorkspace,
    ...
    // ---------------------------------------------------------------------------
    // BackwardBasisStore unit tests (ticket-001)
    // ---------------------------------------------------------------------------

    #[test]
    fn ac_backward_basis_store_new_initializes_all_slots_to_none() {
        let store = BackwardBasisStore::new(5);
        assert_eq!(store.num_stages(), 5);
        for stage in 0..5 {
            assert!(
                store.get(stage).is_none(),
                "read_buf slot {stage} must be None after new()"
            );
        }
        // write_buf is also all-None; verify via write_buf() slice
        assert_eq!(store.write_buf().len(), 5);
        for (stage, slot) in store.write_buf().iter().enumerate() {
            assert!(
                slot.is_none(),
                "write_buf slot {stage} must be None after new()"
            );
        }
    }

    #[test]
    fn ac_backward_basis_store_write_slot_mut_populates_slot() {
        let mut store = BackwardBasisStore::new(3);
        *store.write_slot_mut(1) = Some(CapturedBasis::new(2, 2, 0, 0, 0));
        // write side is populated; read side is still None
        assert!(
            store.write_buf()[1].is_some(),
            "write_buf slot 1 must be Some after write_slot_mut"
        );
        assert!(
            store.get(1).is_none(),
            "read_buf slot 1 must still be None before swap"
        );
    }

    #[test]
    fn ac_backward_basis_store_replace_write_buf_overwrites_write_side() {
        let mut store = BackwardBasisStore::new(3);
        let new_buf = vec![
            Some(CapturedBasis::new(2, 2, 0, 0, 0)),
            None,
            Some(CapturedBasis::new(2, 2, 0, 0, 0)),
        ];
        store.replace_write_buf(new_buf);
        assert!(
            store.write_buf()[0].is_some(),
            "write_buf slot 0 must be Some after replace_write_buf"
        );
        assert!(
            store.write_buf()[1].is_none(),
            "write_buf slot 1 must be None after replace_write_buf"
        );
        assert!(
            store.write_buf()[2].is_some(),
            "write_buf slot 2 must be Some after replace_write_buf"
        );
        // Read side unchanged
        for stage in 0..3 {
            assert!(
                store.get(stage).is_none(),
                "read_buf slot {stage} must remain None after replace_write_buf"
            );
        }
    }

    #[test]
    fn ac_backward_basis_store_swap_buffers_moves_write_to_read_and_clears() {
        let mut store = BackwardBasisStore::new(3);
        // Write a basis into slot 1 of the write buffer
        *store.write_slot_mut(1) = Some(CapturedBasis::new(2, 2, 0, 0, 0));
        store.swap_buffers();
        // After swap: read_buf[1] holds the captured basis
        assert!(
            store.get(1).is_some(),
            "read_buf slot 1 must be Some after swap_buffers"
        );
        // After swap: write_buf[1] is None (cleared)
        assert!(
            store.write_buf()[1].is_none(),
            "write_buf slot 1 must be None after swap_buffers"
        );
        // Other read slots remain None
        assert!(
            store.get(0).is_none(),
            "read_buf slot 0 must be None after swap_buffers"
        );
        assert!(
            store.get(2).is_none(),
            "read_buf slot 2 must be None after swap_buffers"
        );
    }

    #[test]
    fn ac_backward_basis_store_num_stages_returns_ctor_arg() {
        let store = BackwardBasisStore::new(7);
        assert_eq!(store.num_stages(), 7);
        let store_zero = BackwardBasisStore::new(0);
        assert_eq!(store_zero.num_stages(), 0);
    }
}
