# Backward-Basis-Cache Baseline Reference

**Date captured**: 2026-04-20
**Pre-plan commit**: `98ac5c1d933b0b6b51c2ab29a2089eef211acdfe` (`fix: enforce basis validity`)
**Branch at capture**: `feat/backward-basis-cache` (before Epic 01 tickets 001-004 land)

This file is the authoritative regression baseline for:

- **Epic 01, ticket-005** — D-suite + convertido_a byte-identity gate
- **Epic 03, ticket-001** — performance comparison baseline

## EXPECTED_DRIFTS note

Two files from the `convertido_a` case are added to the
`EXPECTED_DRIFTS` allowlist in `scripts/compare_v045_reference.py`
(scoped to `convertido_a` only — the D-suite equivalents remain
byte-identical):

1. `convertido_a/training/cut_selection/iterations.parquet` — cut
   activity columns (`cuts_active_before`, `cuts_deactivated`,
   `cuts_active_after`) differ because the warm-start basis changes LP
   dual choice at LP-degenerate vertices, producing a different sequence
   of cut deactivations. Cut coefficients and intercepts are
   byte-identical. Training lower bound and final policy are unchanged.

2. `convertido_a/training/solver/retry_histogram.parquet` — one extra
   retry event at `iter=8, forward, stage=2, retry_level=0, count=1`
   under the warm-start path. Cascade from the cut-activity drift above.
   The event succeeds immediately (retry_level=0) and does not affect
   cuts.

Rationale documented in `plans/backward-basis-cache/known-concerns.md`
(items R1/R2).

## SHA256 map (pre-plan baseline)

Captured with `scripts/capture_v045_reference.sh target/bwd-cache-baseline target/bwd-cache-baseline/sha256.txt` on commit `98ac5c1`.
...
103518bedb273a96e93258e7724862dc4165e31025ad49671c2ce1613ecfd192  convertido_a/stochastic/inflow_ar_coefficients.parquet
10be86dc6d64327667d9e9bf1303fb8e056ca8175c09936fa0611a097e07f542  convertido_a/stochastic/noise_openings.parquet
...
4a6a5bfb8b70cd6e744757f8ee79af73033a34cb737159eb800c71137c9316ac  convertido_a/training/cut_selection/iterations.parquet
4dd5c0dfbd6cef46ec4c0d578dc4147be0604503d26260460a4fe79bc1fb2c06  convertido_a/stochastic/inflow_seasonal_stats.parquet
...
aca983c099219dd04402068aec7881ff84d079326bfa4de673d868133cc02064  convertido_a/training/dictionaries/bounds.parquet
...
d009bf4ddebcdbc8f2e221f690f01addfcdefcd0e90cf99b6baea107181612f3  convertido_a/training/solver/retry_histogram.parquet
...
## Post-Epic-01 verification (2026-04-20)

Captured on branch `feat/backward-basis-cache` at HEAD with tickets 001-004 applied.

- 27 D-cases: SHA256 byte-identical vs pre-plan baseline.
- convertido_a: 2 drifts (`cut_selection/iterations.parquet`,
  `solver/retry_histogram.parquet`) approved and added to
  `EXPECTED_DRIFTS` (scoped to `convertido_a` only). Cut coefficients,
  intercepts, policy, and lower bound all byte-identical.
- Cross-MPI parity: not run in this session (comparison gate was the
  blocking step resolved here; MPI parity is a separate Epic 01
  acceptance criterion to be verified before the epic-boundary commit).
- `reconstructed_basis_preserves_invariant_on_baked_truncation` test
  passed.
- `test_backward_cache_reduces_pivots` integration test passed.
