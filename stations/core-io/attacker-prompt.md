...
Sweep ONLY these paths (repo-relative; a nested directory is listed with its parent and is in scope): {{SCOPE_PATHS}}
...
3. **Anchors resolve at the baseline with real evidence.** Every anchor `path` is repo-relative and
   must resolve under `git show {{BASELINE_SHA}}:<path>`; every anchor carries a `symbol` (a declared
