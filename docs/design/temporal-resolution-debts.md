...
**Status:** All patterns complete (v0.4.3 + v0.4.4 + feat/temporal-resolution-cd [unreleased])
...
| 2 (observation alignment)     | **Resolved** | unreleased | `0c1aace3` | Layer 2 aggregation implemented; observation-to-season aggregation for Pattern D  |
| 3 (past_inflows metadata)     | **Resolved** | unreleased | `c149c738` | Optional season_ids field on HydroPastInflows for lag-resolution validation       |
| 4 (season_id validation)      | **Resolved** | v0.4.3     | `9c37529`  | Rules 27–30 (range, coverage, consistency, contiguity)                            |
| 5 (noise sharing)             | **Resolved** | unreleased | `7146930c` | Noise group precomputation + ForwardSampler + opening tree sharing                |
| 6 (lag accumulation)          | **Resolved** | v0.4.4     | `97f9655`  | StageLagTransition precomputation + accumulate_and_shift hot-path function        |
...
| 9 (external standardization)  | **Resolved** | v0.4.4     | `8f8cf73`  | standardize_external_inflow uses StageLagTransition for frozen-lag + accumulation |
| 10 (recent_observations)      | **Resolved** | v0.4.4     | `08c1a32`  | RecentObservation type + IO parser + accumulator seeding for mid-season starts    |
| 12 (terminal-stage FCF)       | **Resolved** | v0.4.4     | `667c240`  | BoundaryPolicy config + boundary cut loader + inject_boundary_cuts                |
| 13 (multi-res PAR transition) | **Resolved** | unreleased | `eaa1dd27` | StageLagTransition downstream fields + ring buffer + transition rebuild           |
...
**Pattern B (production DECOMP) is fully resolved as of v0.4.4.**
Debts 6 (lag accumulation), 9 (external standardization),
...
**Pattern C (generic mixed-resolution + PAR) is fully resolved on
feat/temporal-resolution-cd (unreleased).** Debts 5 (noise sharing) and 3
(past_inflows metadata) are closed. D29 integration test verifies weekly
stages with PAR(1) noise sharing.

**Pattern D (monthly-to-quarterly multi-resolution) is fully resolved on
feat/temporal-resolution-cd (unreleased).** Debts 2 Layer 2 (observation
...
functional once feat/temporal-resolution-cd is merged.
...
| 2 (observation aggregation)   | ✅ L1 v0.4.3, L2 unreleased | N/A (no PAR estimation)      | ✅ unreleased       | ✅ unreleased         |
| 3 (past_inflows metadata)     | Low priority                | Low priority                 | ✅ unreleased       | ✅ unreleased         |
| 4 (season_id validation)      | ✅ v0.4.3                   | ✅ v0.4.4                    | ✅ v0.4.3           | ✅ v0.4.3             |
| 5 (noise sharing)             | N/A (monthly stages)        | N/A (External scenarios)     | ✅ unreleased       | ✅ unreleased         |
| 6 (lag accumulation)          | N/A (monthly stages)        | ✅ v0.4.4                    | ✅ v0.4.4           | ✅ v0.4.4             |
| 7 (PAR behavior doc)          | ✅ v0.4.3                   | N/A                          | ✅ v0.4.3           | ✅ v0.4.3             |
| 8 (historical residuals)      | ✅ v0.4.3                   | N/A                          | ✅ v0.4.3           | ✅ v0.4.3             |
| 9 (external standardization)  | N/A (no External)           | ✅ v0.4.4                    | If External used    | If External used      |
| 10 (recent_observations)      | N/A                         | ✅ v0.4.4                    | If mid-season start | N/A                   |
| 12 (terminal-stage FCF)       | N/A                         | ✅ v0.4.4                    | If coupling needed  | If coupling needed    |
| 13 (multi-res PAR transition) | N/A                         | N/A                          | N/A                 | ✅ unreleased         |
...
> **Status: RESOLVED.** Layer 1 validation implemented in v0.4.3 (`316feab`)
> as rule 31: observation-to-season alignment with the same three-tier
> SeasonMap fallback chain used in Debt 1. Layer 2 (fine-to-coarse aggregation
> for Pattern D multi-resolution studies) implemented on feat/temporal-resolution-cd
> (`0c1aace3`): `aggregate_observations_to_season` in `cobre-stochastic/src/par/aggregate.rs`
> performs duration-weighted averaging with leap-year handling. D30 integration
> test verifies monthly-to-quarterly aggregation end-to-end.
...
> **Status: RESOLVED** on feat/temporal-resolution-cd (`c149c738`). Optional
> `season_ids` field added to `HydroPastInflows` in `cobre-core::initial_conditions`.
> IO parser reads and validates `season_ids` when present; semantic validation
> checks consistency between past_inflows lag resolution and the study's PAR
> model resolution. D29 and D30 integration tests exercise the metadata path.
...
> **Status: RESOLVED** on feat/temporal-resolution-cd (`7146930c`).
> `precompute_noise_groups` in `lag_transition.rs` groups stages by
> `(season_id, year)`. The `ForwardSampler` and `generate_opening_tree`
> use noise group IDs to share noise draws across same-group stages via
> `data.copy_within`. D29 integration test verifies weekly stages with
> PAR(1) noise sharing produce consistent inflows.
...
> **Status: RESOLVED** in v0.4.4 (`81776e1`, `97f9655`).
...
> All deterministic regression tests pass unchanged. D28 DECOMP integration
...
> **Status: RESOLVED** in v0.4.4 (`8f8cf73`).
...
> tolerance. All deterministic regression tests pass unchanged.
...
> **Status: RESOLVED** in v0.4.4 (`08c1a32`).
...
> **Status: RESOLVED** in v0.4.4 (`667c240`).
...
> **Status: RESOLVED** on feat/temporal-resolution-cd (`eaa1dd27`).
> `StageLagTransition` extended with `accumulate_downstream`, `downstream_finalize`,
> and `rebuild_from_downstream` fields. `DownstreamAccumState` ring buffer in
> `ScratchBuffers` accumulates completed quarterly lags during the monthly phase.
> At the transition stage, `rebuild_from_downstream` overwrites the primary lag
> state from the ring buffer. `compute_downstream_transitions` in `lag_transition.rs`
> precomputes the downstream window. D30 integration test verifies monthly-to-quarterly
> multi-resolution with observation aggregation and lag transition.
