...
SUB_RE = re.compile(r"[A-Za-z0-9][A-Za-z0-9-]*")
PERF_LENSES = {"perf", "performance"}
PARTIREF_RE = re.compile(r"I\.(?:3-[1-9]|5)")
SEV = {"A", "B", "C"}
ALIGN = {"advances-0a", "advances-0b", "advances-1", "neutral", "conflicts"}
VERDICT = {"confirmed", "dismissed"}
DIFF_MARKERS = ("---", "+++", "@@", "diff ", "```")
TIMING_RE = re.compile(
    r"\b\d+(?:\.\d+)?\s*(?:ms|µs|us|ns|s|sec|secs|seconds|minutes|min)\b", re.I
)


def validate_attacker(
    env: dict[str, Any], errs: list[str], station: str | None
) -> None:
    for key in (
        "station",
        "subStation",
    ...
    if not SUB_RE.fullmatch(str(env.get("subStation", ""))):
        errs.append(
            f"$.subStation: {env.get('subStation')!r} is not a sub-station token"
        )
    ...
    for i, cand in enumerate(env.get("candidates") or []):
        ...
        for j, anc in enumerate(anchors or []):
            ...
            if anc.get("line") is not None and (
                not isinstance(anc["line"], int) or anc["line"] < 1
            ):
        ...
        if (
            fix.lstrip().startswith(DIFF_MARKERS)
            or "\n+" in fix
            or "\n-" in fix
            or "```" in fix
        ):
            errs.append(f"{at}.fixShape: looks like a diff or a code block; prose only")
        if cand.get("partIRef") is not None and not PARTIREF_RE.fullmatch(
            str(cand["partIRef"])
        ):
            errs.append(f"{at}.partIRef: not an I.3-n / I.5 reference")
        if env.get("lens") in PERF_LENSES:
            if not str(cand.get("mechanism", "")).strip():
                errs.append(
                    f"{at}.mechanism: perf candidates must state the cost mechanism"
                )
            blob = (
                " ".join(
                    str(cand.get(k, "")) for k in ("title", "mechanism", "fixShape")
                )
                + " "
                + str((ev or {}).get("reading", ""))
            )
            if TIMING_RE.search(blob):
                errs.append(
                    f"{at}: perf candidates stay UNMEASURED; a timing number was found"
                )
...
def validate_defender(
    ...
    for i, ver in enumerate(env.get("verdicts") or []):
        ...
        if ver.get("verdict") == "dismissed" and not (
            ver.get("argument") or ver.get("sanctionedBy")
        ):
            errs.append(f"{at}: dismissal needs argument or sanctionedBy")
        if (
            ver.get("verdict") == "confirmed"
            and not str(ver.get("survivingClaim", "")).strip()
        ):
            errs.append(
                f"{at}.survivingClaim: a confirmed verdict needs a surviving claim"
            )
